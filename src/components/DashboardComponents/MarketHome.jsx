...
// NOTE: In the original file imports for Button / Card / Switch were failing because those
// files didn't exist at the referenced paths in the user's environment. To make this file
// runnable immediately we provide small local fallback implementations for Button, Card,
// CardContent and Switch. In the global project we can remove these local fallbacks and
// restore the project-wide shadcn/ui components once their paths are correct.
...
function Switch({ checked, onCheckedChange }) {
  // simple accessible checkbox styled as a switch
  return (
    <button
      role="switch"
...
function Sidebar({ collapsed, onToggle, isSellerView, onToggleView }) {
  ...
  return (
    <aside className={`transition-all duration-300 ${isSellerView ? 'bg-[#31587c] text-white' : 'bg-[#1CA6AF] text-white'} shadow-lg ${collapsed ? 'w-20' : 'w-72'} h-screen sticky top-0 flex flex-col`}>
...
export default function Dashboard() {
  ...
  // minimal demo content so file runs without external APIs
  const demoBuyCard = (
    <Card className="p-6 bg-[#1CA6AF]/10">
      <CardContent>
        <h2 className="text-xl font-semibold mb-2 text-[#1CA6AF]">Buyer Dashboard</h2>
        <p className="text-gray-700">Explore and purchase SaaS apps easily.</p>
      </CardContent>
    </Card>
  );

  const demoSellerCard = (
    <Card className="p-6 bg-[#31587c]/10">
      <CardContent>
        <h2 className="text-xl font-semibold mb-2 text-[#31587c]">Seller Dashboard</h2>
        <p className="text-gray-700">Manage listings, payments, and collaborations.</p>
      </CardContent>
    </Card>
  );
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.25 }} className="min-h-screen flex bg-gray-50">
      ...
      <main className="flex-1 p-6 overflow-y-auto">
        ...
        <div className="grid gap-4">
          {isSellerView ? demoSellerCard : demoBuyCard}
        </div>
      </main>
    </motion.div>
  );
