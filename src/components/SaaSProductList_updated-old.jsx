...
function SaaSProductListInner({ query = "", onPreview: extPreview, onBuy: extBuy }) {
  const { startCheckout } = useCheckout();
...
SaaSProductListInner.propTypes = {
  query: PropTypes.string,
  onPreview: PropTypes.func,
  onBuy: PropTypes.func,
};

// Wrapped version with providers
export default function SaaSProductList(props) {
  return (
    <AppProviders>
      <SaaSProductListInner {...props} />
    </AppProviders>
  );
}
...
function ModalConsumer() {
  ...
  if (type === "preview") {
    const { product } = props;
    return <PreviewModal product={product} open={true} onClose={closeModal} onBuy={() => {}}
    />;
  }
    ...
    return (
      <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
        <div className="bg-white p-6 rounded-xl shadow">
          <h3 className="font-semibold">Checkout started (mock)</h3>
          <p className="text-sm text-gray-600 mt-2">Your mock Stripe checkout has been initiated. Close to continue.</p>
